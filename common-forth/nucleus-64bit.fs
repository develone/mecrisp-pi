...
header-1-foldable 1-       :noname     1-       ;
header @                   : @         h# 4000000000000000 or >r ;
...
header-1-foldable cell+    : cell+     d# 8 + ;
...
: abs ( n -- |n| )
  d# 63 overswaparshift ( n sign )
  swap ( sign n )
  over+ ( sign n+sign )
  xor
;

header-1-foldable cells    :noname     d# 3 lshift ;
...
\ -----------------------------------------------------------------------------
\  Word, halfword and byte memory access, emulated
\ -----------------------------------------------------------------------------

header w@
: w@
    dup @ swap
    d# 4 and if d# 32 rshift then
    h# FFFFFFFF and
;
...
header .x4 : .x4 ( x -- ) hex4 space ;
header .x8 : .x8 ( x -- ) hex8 space ;
header .x  : .x  ( x -- ) hex16 space ;
...
\ 8 bytes link and foldability:  61 bits link, 3 bits foldability.
...
header link@
: link@
    @ 2/ 2/ h# 7FFFFFFFFFFFFFF8 and \ Remove flags for foldability.
;
...
: sameword ( c-addr u wp -- c-addr u wp flag )
    2dup cell+ c@ d# 127 and = if              \ lengths match?
        3rd 3rd 3rd                 \ 3dup
        d# 9 + >r                   \ R: word in dictionary
        bounds
        begin
            2dupxor
        while
            dup c@ r@ c@ i<> if
                drop droprdrop false ;
            then
            1+
            r> 1+ >r
        repeat
...
: compile,
  ...
  if
    ...
    if ( call-target )
      ...
      if
         \ High-Call opcode to fetch variable:
         h# C000000000000000 prev@xor \ Add $4000000000000000 to address and remove MSB which denotes literal opcodes
         d# 3 rshift
         h# 4000000000000000 or       \ Make call opcode
         prev _!
         jmp tdrop
      then
    then
  ...
  if
    dup cell+ @ h# 600000000000008c = \ Which is directly followed with an exit ?
    if
      @ jmp w, \ Inline it !
    then
  then

  dup @ h# e000000000000080 and h# 6000000000000080 = \ ALU Opcode with ret bits set ?
  if  @ h# ffffffffffffff73 and \ Inline it with ret bits removed.
  else
    d# 3 rshift h# 4000000000000000 or \ Compile classically as a simple call opcode
  then
  w,
;
...
: texit
    ...
    if
      h# 4000000000000000 prev@xor
      h# e000000000000000 overand
      if
        drop
      else
        prev jmp _!
      ...
      prev @ h# e00000000000008c and h# 6000000000000000 = \ ALU opcode with neither ret nor return stack in opcode ?
      if
        h# 000000000000008C prev@xor prev jmp _!
      then
...
header-imm ahead
: tahead
    here h# 0000000000000000 w,
;

header-imm if
: tif
    here h# 2000000000000000 w,
;
...
header decimal : decimal ( -- ) d# 10 : setbase base _! ;
