...
header-1-foldable cell+    : cell+     d# 4 + ;
...
: abs ( n -- |n| )
  d# 31 overswaparshift ( n sign )
  swap ( sign n )
  over+ ( sign n+sign )
  xor
;

header-1-foldable cells    :noname     2* 2*    ;
...
\ -----------------------------------------------------------------------------
\  Word and byte memory access, emulated
\ -----------------------------------------------------------------------------
...
\ 4 bytes link and foldability:  29 bits link, 3 bits foldability.
...
header link@
: link@
    @ 2/ 2/ h# 7FFFFFFC and \ Remove flags for foldability.
;
...
: compile,
  ...
  if
    ...
    if ( call-target )
      ...
      if
         ...
         h# C0000000 prev@xor \ Add $40000000 to address and remove MSB which denotes literal opcodes
         2/ 2/
         h# 40000000 or       \ Make call opcode
         prev _!
         jmp tdrop
      then
    then
  ...
  if
    dup cell+ @ h# 6000008c = \ Which is directly followed with an exit ?
    if
      @ jmp w, \ Inline it !
    then
  then

  dup @ h# e0000080 and h# 60000080 = \ ALU Opcode with ret bits set ?
  if  @ h# ffffff73 and \ Inline it with ret bits removed.
  else
    2/ 2/ h# 40000000 or \ Compile classically as a simple call opcode
  then
  w,
;
...
: texit
    ...
    if
      ...
      prev @ h# e000008c and h# 60000000 = \ ALU opcode with neither ret nor return stack in opcode ?
      if
        h# 0000008C prev@xor prev jmp _!
      then
